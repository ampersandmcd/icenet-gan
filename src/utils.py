"""
Adapted from Tom Andersson's https://github.com/tom-andersson/icenet-paper/blob/main/icenet/utils.py
and from Joshua Dimasaka, Andrew McDonald, Meghan Plumridge, Jay Torry, and
Andrés Camilo Zúñiga González's https://github.com/ai4er-cdt/sea-ice-classification/blob/main/util.py
Modified the former from Tensorflow to PyTorch.
Modified the latter from segmentation to forecasting.
"""
import os
import numpy as np
from torch.utils.data import Dataset
from src.models import linear_trend_forecast
...
class Visualise(Callback):
    """
    Callback to visualise prediction/target triplets.
    """
    def __init__(self, dataloader, n_to_visualise, n_forecast_months):
        """
        Construct callback object.
        """
        self.dataloader = dataloader
        self.n_to_visualise = n_to_visualise
    ...
    def on_validation_epoch_start(self, trainer, pl_module):
        ...
        for batch in self.dataloader:
            ...
            y_bin = (y.argmax(dim=-2) > 0).long()  # sum marginal and full ice for binary eval
            y_hat = pl_module(x.to(pl_module.device))
            y_hat_bin = (y_hat.argmax(dim=-2) > 0).long()  # sum marginal and full ice for binary eval
            n_rows = self.n_to_visualise * 4  # preds, mask, preds with mask, truth for each sample
            n_cols = 6  # TODO: fix self.n_forecast_months below from hardcoded 6 # one column for each lead time
            fig, ax = plt.subplots(n_rows, n_cols, figsize=(n_cols * 3, n_rows * 4))
            for i in range(n_rows):
                for j in range(n_cols):
                    if i % 4 == 0:
                        ax[i, j].imshow(y_hat_bin[i//4, :, :, j].detach().cpu().numpy(), 
                                        vmin=0, vmax=1)
                        ax[i, j].set_title(f"Binary Prediction\n@ Lead {j+1}")
                    elif i % 4 == 1:
                        ax[i, j].imshow(sample_weight.squeeze()[i//4, :, :, j].detach().cpu().numpy(),
                                        vmin=0, vmax=1)
                        ax[i, j].set_title(f"Sample Weight\n@ Lead {j+1}")
                    elif i % 4 == 2:
                        ax[i, j].imshow(y_hat_bin[i//4, :, :, j].detach().cpu().numpy() \
                                        * (sample_weight.squeeze()[i//4, :, :, j].detach().cpu().numpy() > 0),
                                        vmin=0)
                        ax[i, j].set_title(f"Masked Binary Prediction\n@ Lead {j+1}")
                    elif i % 4 == 3:
                        ax[i, j].imshow(y_bin[i//4, :, :, j].detach().cpu().numpy(),
                                        vmin=0, vmax=1)
                        ax[i, j].set_title(f"Binary Truth\n@ Lead {j+1}")
            plt.tight_layout()
    ...
    def on_test_epoch_start(self, trainer, pl_module):
        """
        Callback to run on test epoch start.
        :param trainer: PyTorch Lightning Trainer class instance
        :param pl_module: PyTorch Lightning Module class instance
        """
        for batch in self.dataloader:
            ...
            n_rows = self.n_to_visualise * 2  # preds in one row, truth in one row, for each sample
            n_cols = y_hat_pred.shape[-1],  # one column for each lead time
            fig, ax = plt.subplots(n_rows, n_cols, figsize=(n_rows * 3, n_cols * 3))
            for i in range(n_rows):
                for j in range(n_cols):
                    if i % 2 == 0:
                        ax[i, j].imshow(y_hat_pred[i].detach().cpu().numpy(), 
                                        vmin=0, vmax=pl_module.model.n_output_classes)
                        ax[i, j].set_title("Prediction")
                    else:
                        ax[i, j].imshow(y[i].detach().cpu().numpy(),
                                        vmin=0, vmax=pl_module.model.n_output_classes)
                        ax[i, j].set_title("Truth")
            plt.tight_layout()
            wandb_logger = trainer.logger.experiment
            wandb_logger.log({"val_image": fig})
            plt.close(fig)
...
class IceNetDataset(Dataset):
    ...
    training IceNet. Converted from original Tensorflow implementation.
    Inherts from torch.utils.data.Dataset class for integration in PyTorch.
    Must implement a __len__ method that returns the number of batches 
    and a __getitem__ method that returns a batch of data. 
    The on_epoch_end method is called after each epoch.
    See: https://towardsdatascience.com/a-tale-of-two-frameworks-985fa7fcec
    See: https://pytorch.org/docs/stable/data.html#torch.utils.data.Dataset
    """

    def __init__(self, dataloader_config_fpath, mode="train", seed=None):
        '''
        Params:
        dataloader_config_fpath (str): Path to the data loader configuration
            settings JSON file, defining IceNet's input-output data configuration.

        mode (str): Sets the dataset to be "train", "val", or "test" mode based
            on the dates provided in the dataloader configuration json
        ...
        self.set_obs_forecast_IDs(dataset=mode)
    ...
    def data_generation(self, forecast_IDs):
        ...
        return X.squeeze(0), y.squeeze(0), sample_weight.squeeze(0)  # if batch_size == 1 collapse batch dimension
