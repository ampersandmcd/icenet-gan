"""
Courtesy of Tom Andersson via https://github.com/tom-andersson/icenet-paper/blob/main/icenet/rotate_wind_data.py
...
def download_seas5_forecasts(parallel=False):
    """
    Download SEAS5 forecasts for multiple leadtimes.

    :param parallel: Whether to use multiprocessing to launch multiple downloads at once.
    """
    leadtimes = [1, 2, 3, 4, 5, 6]
    if parallel:
        with Pool(len(leadtimes)) as p:
            p.map(download_seas5_forecast, leadtimes)
    else:
        for leadtime in leadtimes:
            download_seas5_forecast(leadtime)


def download_seas5_forecast(leadtime):
